...
public class NeuralNet1 {
    ...
    //初始化
    public NeuralNet1(int inputCount, int hidderCount, int outputCount){
    ...
    public void backPropagation(){
        ...
        for(int i=0;i<hiddenLayerErr.length;i++){
            for(int j=0;j<outputLayerErr.length;j++){
                ...
                hiddenLayerErr[i]+=outputLayerErr[j]* outputLayerWeight[j][i]*sigmod_prime(hiddenLayerZ[i]);
            }
        }
        for(int i=0;i<outputLayerWeight.length;i++){
            for(int j=0;j<outputLayerWeight[i].length;j++){
    ...
    public static void main(String[] args){
        NeuralNet1 neuralNet=new NeuralNet1(2,10,1);

        for (int c = 0; c < 100000; c++) {
            int i = Math.random() > 0.5 ? 1 : 0;
            int j = Math.random() > 0.5 ? 1 : 0;
            double[] inputLayer = new double[] {i, j};
            double[] outputLayerExpect = new double[] {i + j > 1 ? 1 : 0};
        ...
        System.out.println("训练完成");
        {
            ...
            double[] outputLayerExpect = new double[] {i + j > 1 ? 1 : 0};
            neuralNet.forward(inputLayer, outputLayerExpect);
            System.out.println("输出为"+neuralNet.outputLayerA[0]+"/期望为"+outputLayerExpect[0]);
        }
        {
            int i = 0;
            int j = 0;
